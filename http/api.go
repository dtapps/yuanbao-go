...
// GetSignToken 获取签票
func (c *Client) GetSignToken() (*types.AuthResult, error) {
	// 检查缓存
	cache := account.GetTokenCacheManager()
	...
	for attempt := 0; attempt <= SignMaxRetries; attempt++ {
		...
		// 缓存Bot ID
		if result.BotID != "" {
			account.GetManager().SetBotId(c.account.AccountID, result.BotID)
		}
...
func (c *Client) ForceRefreshSignToken() (*types.AuthResult, error) {
	cache := account.GetTokenCacheManager()
...
func (c *Client) doFetchSignToken() (*types.AuthResult, error) {
...
func (c *Client) computeSignature(nonce, timestamp string) string {
...
func (c *Client) YuanbaoPost(path string, body any) ([]byte, error) {
	return c.doRequest("POST", path, body)
}

// YuanbaoGet 发送GET请求
func (c *Client) YuanbaoGet(path string, params map[string]string) ([]byte, error) {
...
func (c *Client) doRequest(method, path string, body any) ([]byte, error) {
	...
	for attempt := 0; attempt <= HTTPAuthRetryMax; attempt++ {
		...
		if resp.StatusCode == http.StatusUnauthorized && attempt < HTTPAuthRetryMax {
			...
			account.GetTokenCacheManager().Clear(c.account.AccountID)
			continue
		}
